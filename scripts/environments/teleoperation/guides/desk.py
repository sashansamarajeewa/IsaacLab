...
class DeskGuide(BaseGuide):
    ...
    def on_reset(self, env):
        ...
        if (
            ...
            self._target_poses["DeskTop"] = (self.tgt_box_pos, self.tgt_box_quat)

            # target FrontLeftLeg inserted to DeskTop
            self._target_poses["FrontLeftLeg"] = (self.tgt_bot_pos, self.tgt_bot_quat)
            
            # target FrontRightLeg inserted to DeskTop
            self._target_poses["FrontRightLeg"] = (self.tgt_bot_pos, self.tgt_bot_quat)

            # target BackLeftLeg inserted to DeskTop
            self._target_poses["BackLeftLeg"] = (self.tgt_top_pos, self.tgt_top_quat)
            
            # target BackRightLeg inserted to DeskTop
            self._target_poses["BackRightLeg"] = (self.tgt_top_pos, self.tgt_top_quat)
